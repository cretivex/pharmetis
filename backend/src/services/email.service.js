...
export const sendPasswordResetLinkEmail = async (email, resetLink) => {
  const subject = 'Reset Your Password - Phartimetic';
  const html = `
    <!DOCTYPE html>
    <html>
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827; max-width: 600px; margin: 0 auto; padding: 20px;">
      ...
      <p style="margin: 20px 0;">
        <a href="${resetLink}" style="background:#0f172a;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;display:inline-block;">
          Reset password
        </a>
      </p>
      <p style="word-break: break-all;">Or open this link directly: <a href="${resetLink}">${resetLink}</a></p>
...
export const queueEmailSend = (promiseFactory, context = {}) => {
  const enqueue = typeof setImmediate === 'function'
    ? setImmediate
    : (cb) => setTimeout(cb, 0);
...
export const sendRFQAcceptedEmail = async ({ to, buyerName, rfqId, supplierName }) => {
  const template = rfqAcceptedTemplate(buyerName, rfqId, supplierName);
  return sendEmail({ to, ...template });
};

export const sendRFQRejectedEmail = async ({ to, buyerName, rfqId, supplierName }) => {
  const template = rfqRejectedTemplate(buyerName, rfqId, supplierName);
  return sendEmail({ to, ...template });
};

export const sendRFQQuotedEmail = async ({ to, buyerName, rfqId, supplierName, price }) => {
  const template = rfqQuotedTemplate(buyerName, rfqId, supplierName, price);
  return sendEmail({ to, ...template });
};

export const sendRFQAcceptedAdminEmail = async ({
  to,
  rfqId,
  buyerName,
  buyerEmail,
  ...
  const subject = 'Buyer accepted quotation - Action confirmation';
  const adminLink = `https://phartimetic.com/admin/rfq/${rfqId}`;
  const html = `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #111827; max-width: 640px; margin: 0 auto; padding: 20px;">
      <h2 style="margin: 0 0 12px;">Buyer accepted quotation</h2>
      <p style="margin: 0 0 12px;">A buyer has accepted a quotation. Please proceed with confirmation and follow-up.</p>
      <p><strong>RFQ ID:</strong> ${rfqId}</p>
      <p><strong>Buyer Name:</strong> ${buyerName || 'Buyer'}</p>
      <p><strong>Buyer Email:</strong> ${buyerEmail || 'N/A'}</p>
      <p><strong>Supplier:</strong> ${supplierName || 'N/A'}</p>
      <p><strong>Accepted Amount:</strong> ${acceptedAmount != null ? acceptedAmount : 'N/A'}</p>
      <p style="margin-top: 16px;">
        <a href="${adminLink}" style="display: inline-block; background: #0f172a; color: #ffffff; text-decoration: none; padding: 10px 16px; border-radius: 8px;">
          Review RFQ
        </a>
      </p>
    </body>
  ...
  `;
  const text = `Buyer accepted quotation\n\nRFQ ID: ${rfqId}\nBuyer Name: ${buyerName || 'Buyer'}\nBuyer Email: ${buyerEmail || 'N/A'}\nSupplier: ${supplierName || 'N/A'}\nAccepted Amount: ${acceptedAmount != null ? acceptedAmount : 'N/A'}\nReview RFQ: ${adminLink}`;
