...
import { queueEmailSend, sendRFQQuotedEmail } from '../../services/email.service.js';
...
export const createRFQResponseService = async (rfqId, userId, data) => {
  ...
  try {
    ...
    // NEGOTIATION_REQUESTED must use resubmit endpoint, not create
    ...
    // One submission per RFQ per supplier: only DRAFT may be edited via this endpoint.
    // After first submit (SUBMITTED+), supplier cannot change line items here — use admin "send back" + resubmit when allowed.
    if (existingResponse && existingResponse.status !== 'DRAFT') {
      const useResubmitStatuses = ['SENT_BACK_TO_SUPPLIER', 'REVISION_REQUESTED', 'REJECTED'];
      if (useResubmitStatuses.includes(existingResponse.status)) {
        throw new ApiError(
          400,
          'Please use the resubmit quotation flow to send a revised quote for this RFQ.'
        );
      }
      throw new ApiError(
        400,
        'You have already submitted a quotation for this RFQ. Further changes are not allowed.'
      );
    }

    // DRAFT only: update existing response instead of creating a duplicate row
    if (existingResponse) {
      logger.info(`Updating draft RFQ response ${existingResponse.id} for RFQ ${rfqId}, Supplier ${supplier.id}`);
...
export const resubmitQuotationService = async (id, userId, data) => {
  try {
    ...
    const resubmitAllowed = ['REJECTED', 'SENT_BACK_TO_SUPPLIER', 'REVISION_REQUESTED'];
    if (!resubmitAllowed.includes(existingResponse.status)) {
      throw new ApiError(400, `Cannot resubmit quotation with status: ${existingResponse.status}. Only rejected or returned quotations can be resubmitted.`);
    }
...
export const sendQuotationToBuyerService = async (id, user, editedData = null) => {
  try {
    ...
    const buyerEmail = response.rfq?.buyer?.email;
    if (buyerEmail) {
      queueEmailSend(
        () => sendRFQQuotedEmail({
          to: buyerEmail,
          buyerName: response.rfq?.buyer?.fullName,
          rfqId: response.rfqId,
          supplierName: response.supplier?.companyName,
