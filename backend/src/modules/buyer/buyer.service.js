...
export const createBuyerRFQService = async (buyerId, data) => {
  try {
    ...
    if (uniqueProductIds.length !== items.length) {
      throw new ApiError(400, 'Each RFQ line must reference a valid catalog product');
    }
    ...
    if (products.length !== uniqueProductIds.length) {
      throw new ApiError(400, 'Some selected products are unavailable. Please choose only available catalog products.');
    }
    ...
    const adminRecipients = String(env.ALERT_EMAIL_TO || '')
      .split(',')
    ...
    if (adminRecipients.length > 0) {
      ...
      const buyerName = buyer?.fullName || 'Buyer';
      const buyerEmail = buyer?.email || 'N/A';
      const rfqLink = `https://phartimetic.com/admin/rfq/${rfq.id}`;
      const subject = 'New RFQ Submitted - Admin Review Required';
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 640px; margin: 0 auto; color: #0f172a; line-height: 1.6;">
          <h2>New RFQ Submitted</h2>
          <p>A new RFQ was submitted and is ready for admin review.</p>
          ...
          <p style="margin-top: 18px;">
            <a href="${rfqLink}" style="background:#0f172a;color:#fff;text-decoration:none;padding:10px 14px;border-radius:8px;display:inline-block;">
              Review RFQ
            </a>
          </p>
        </div>
      `;
      const text = `New RFQ Submitted\n\nRFQ ID: ${rfq.id}\nTitle: ${rfqTitle}\nBuyer Name: ${buyerName}\nBuyer Email: ${buyerEmail}\nItems: ${items.length}\nReview RFQ: ${rfqLink}`;

      queueEmailSend(
        () => sendEmail({ to: adminRecipients.join(','), subject, html, text }),
        {
          event: 'rfq.created.admin_alert',
          rfqId: rfq.id,
          to: adminRecipients
        }
      );
...
export const acceptBuyerQuotationService = async (quotationId, buyerId) => {
  try {
    ...
    const buyerEmail = quotation.rfq?.buyer?.email;
    if (buyerEmail) {
      queueEmailSend(
        () => sendRFQAcceptedEmail({
          to: buyerEmail,
          buyerName: quotation.rfq?.buyer?.fullName,
          rfqId: quotation.rfqId,
    ...
    if (adminRecipients.length > 0) {
      queueEmailSend(
        () => sendRFQAcceptedAdminEmail({
          ...
          buyerEmail,
          supplierName: quotation.supplier?.companyName,
          acceptedAmount: finalPrice
        }),
        {
          event: 'rfq.accepted.admin_alert',
          rfqId: quotation.rfqId,
          quotationId,
          to: adminRecipients
        }
