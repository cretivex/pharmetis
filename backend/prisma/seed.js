...
async function main() {
  ...
  for (let i = 0; i < productData.length; i++) {
    ...
    const ndcDefault = `16729-${String(331 + i).padStart(3, '0')}-01`;
    const packDefault =
      data.packagingType?.includes('90') && !data.packagingType.includes('100')
        ? '90 ct'
        : data.packagingType?.includes('14')
          ? '14 ct'
          : '100 ct';
    ...
    if (existing) {
      ...
      product = await prisma.product.create({
        data: {
          ...
          ndcNumber: data.ndcNumber ?? ndcDefault,
          packSize: data.packSize ?? packDefault,
