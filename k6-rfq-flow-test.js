...
export function setup() {
  ...
  if (!check(productsRes, { 'products setup 200': (r) => r.status === 200 })) {
    fail(`products setup failed: ${productsRes.body}`);
  }
  ...
  if (!check(suppliersRes, { 'suppliers setup 200': (r) => r.status === 200 })) {
    fail(`suppliers setup failed: ${suppliersRes.body}`);
  }
...
export default function (data) {
  ...
  const rfqPayload = {
    ...
    notes: 'Automated k6 RFQ flow test',
    items: [
      {
  ...
  // Admin -> Margin/Review + Send to buyer
  const review = http.patch(
    `${BASE_URL}/api/rfq-responses/${quoteId}/review`,
    JSON.stringify({ action: 'APPROVE', adminNotes: 'Approved by k6 flow (margin handled in admin flow)' }),
